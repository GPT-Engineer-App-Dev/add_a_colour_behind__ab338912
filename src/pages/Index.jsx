        ...
        <Table variant="simple">
          ...
          <Tbody>
            {Array.from({ length: 18 }, (_, holeIndex) => (
              <Tr key={holeIndex} bg={currentHole === holeIndex ? "yellow.100" : "white"}>
                <Td isNumeric color={currentHole > holeIndex ? "gray.500" : "black"}>{`Hole ${holeIndex + 1}`}</Td>
                {players.map((player, playerIndex) => (
                  <Td key={playerIndex} isNumeric color={currentHole > holeIndex ? "gray.500" : "black"}>
                    <Input
                      type="number"
                      value={player.scores[holeIndex] === 0 ? "" : player.scores[holeIndex]}
